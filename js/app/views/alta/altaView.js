define([
	...
	'funciones',
	'app/formulario',
	'app/views/alta/html/altaViewHtml',
...
], function( $, _, Backbone, Config, Templates, Merca, Fx, Formulario, AltaViewHtml ){

	'use strict';

	var AltaView = Backbone.View.extend({

		html: AltaViewHtml['alta'],
		...
		*/
		...
		events: {
			...
			//'click .tab-nav a'                         : 'check_iso',	// Cancelado en Merca
		...
		*/
		...
		render: function(){
			...
			*/
			...
			// paises
			$.each( Config.obj_paises, function( indice, valor ){

				if(valor != ''){
					html_paises += '<div class="checkbox m-b-15 col-sm-3 col-lg-3">';
	                html_paises += '<label>';
	                html_paises += '<input type="checkbox" value="' + valor + '">';
	                html_paises += '<i class="input-helper"></i>';
	                html_paises += valor;
	                html_paises += '</label>';
	                html_paises += '</div>';
			...
			this.$('#div_paises').html( html_paises );
		...
		actualizar_obj_cliente: function(){
			...
			this.obj_cliente = Merca.actualizar_obj_form( this.$('#form_anadir_cliente_web') );
			...
			this.obj_cliente.frescos = this.crear_ocupaciones_empresa( 'frescos' );
			this.obj_cliente.envasados = this.crear_ocupaciones_empresa( 'envasados' );
			this.obj_cliente.provincias = this.crear_ocupaciones_empresa( 'provincias' );
			this.obj_cliente.paises = this.crear_ocupaciones_empresa( 'paises' );
		...
		guardar_cliente: function(){
			...
			Merca.spinner( this, '#resp_guardar_cliente');
		...
		crear_ocupaciones_empresa: function(grupo_check){

			var num_checked = 0, arr_datos = [];

			$.each( this.$('#' + grupo_check + ' input[type="checkbox"]'), function(index, value) {

				if( $(this).is(':checked') ){
					
					arr_datos.push( $(this).val() );
					num_checked ++;
				}
			...
			if( num_checked == 52 ){ // Seleccionarmos todas las provincias
